			<section class="uk-section">
				<div class="uk-container uk-container-small">
					...
					?>
					<form action="/berita/edit/<?php echo $beritaid; ?>" name="updateberita" class="uk-form uk-form-stacked" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
						<div class="uk-margin">
							<label class="uk-form-label">Judul</label>
							<div class="uk-form-controls">
								<input class="uk-form-input uk-width-1-1 uk-form-large" type="text" name="judul" <?php if (isset ($post['judul'])) { echo 'value="'. $post['judul'] .'"'; } if (isset ($berita['title'])) { echo 'value="'. $berita['title'] .'"'; } ?> required/>
							</div>
						</div>
						<div class="uk-margin">
							...
							<div id="fotodisplay" class="uk-child-width-1-2" uk-grid>
								<?php
									...
									{
										echo '<div id="foto-display" class="uk-inline">';
										echo '<img class="uk-width-1-1" src="https://mataramutama.com/images/berita/' .$berita['foto']. '"/>';
										echo '<a class="uk-position-small uk-position-top-right uk-light" style="padding: 5px; background-color: #f0506e; border-radius: 15px; line-height: 15px;" onclick="takeoutfoto()"><span uk-icon="close"></span></a>';
										echo '</div>';
									}
								?>
							...
							<div id="fotolist">
								<?php
									...
									if (isset ($berita['foto']))
									{
										echo '<input class="input-foto" id="foto" name="foto" value="' .$berita['foto']. '" hidden="hidden">';
									}
								?>
							</div>
							<script>
						...
						<div class="uk-margin">
							...
							<div class="uk-form-controls">
								<textarea class="uk-textarea" rows="5" name="intro" placeholder="Tuliskan paragraf pertama berita di sini" required><?php if (isset ($post['intro'])) { echo $post['intro']; } if (isset ($berita['intro'])) { echo $berita['intro']; } ?></textarea>
							</div>
						</div>
						<div class="uk-margin">
							<label class="uk-form-label">Konten Berita</label>
							<div class="uk-form-controls">
								<textarea id="beritainput" name="content"><?php if (isset ($post['content'])) { echo $post['content']; } if (isset ($berita['konten'])) { echo $berita['konten']; } ?></textarea>
								<script>
									tinymce.init({
